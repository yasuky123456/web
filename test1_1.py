...
if sideradio == 'MR':
    ...
    #rogin_name = st.text_input('★step1.Userログイン名を入力','例.koya')
    rogin_name = st.selectbox('★step1.Userログイン名を選択',('koya','yasu','chaen','niwa','その他'))

    if rogin_name == 'その他':
        rogin_name = st.text_input('★step1_1.ログイン名入力','例.koya')


    st.write('★step2.DesktopにSNR_DICOMフォルダ作成します.2回目以降不要')
    if st.button('SNR_DICOMフォルダ作成'):

        #pre_path = "C:\\Users\\" + rogin_name +"\\Desktop"
        path ="C:\\Users\\" + rogin_name +"\\Desktop"
        dir_main = 'SNR_DICOM'
        dir_list = ['1回目','2回目']

        #st.write(path)

        for path_lis in dir_list:
            new_path = path + "\\" + dir_main + "\\" + path_lis
            os.makedirs(new_path, exist_ok=True)
            #st.write(new_path)
        

    st.write('★step3. DATAをSNR_DICOMに入れてください(とりあえず50個まで)')
    
    if st.button('入力データ確認'):
        ...
        files_1 = sorted(glob.glob("C:\\Users\\"+rogin_name+"\\Desktop\\SNR_DICOM\\1回目\\*.dcm"), key=natural_keys)
        #順番確認
        st.write("1回目フォルダ入力")
        st.write(files_1)
        #st.write(rogin_name)
        ...
        files_2 = sorted(glob.glob("C:\\Users\\"+rogin_name+"\\Desktop\\SNR_DICOM\\2回目\\*.dcm"), key=natural_keys)
        #順番確認
        st.write("2回目フォルダ入力")
        st.write(files_2)
    ...
    if st.button('ROI設定とSNR結果一覧'):
        ...
        path_img = "C:\\Users\\"+rogin_name+"\\Desktop\\SNR_DICOM"
        ...
        for i in files_1 :
            ...
            #ROI設定
            img_ROI = cv2.rectangle(pixels, (I,I), (II, II), (255, 255, 255), thickness=-1)
            cv2.imwrite(path_img + "\\SNR.jpg", img_ROI)

            #画像表示

        image = Image.open("C:\\Users\\"+rogin_name+"\\Desktop\\SNR_DICOM\\SNR.jpg")
        ...
        datas_1 = []
        for i in files_1 :
            img_1 = pydicom.dcmread(i)
            pixels_1 = img_1.pixel_array
            data_df_1 = pd.DataFrame(pixels_1)
            datas_1.append(data_df_1)
        df_1 = pd.concat(datas_1,axis=0)
        #pd作成時，unit16になるためfloat64を指定して読み込み
        df_1 = df_1.astype("float64")

        #チェック
        #check = st.button('df_1csv出力')

        #if check == True:
        #    df_1.to_csv('C:\\Users\\koya\\Desktop\\df_1.csv',header=False,index = False, encoding='utf-8')

        datas_2 = []
        for j in files_2 :
            img_2 = pydicom.dcmread(j)
        ...
        #チェック
    #    check = st.button('df_2csv出力')
    #   if check == True:
    #        df_2.to_csv('C:\\Users\\koya\\Desktop\\df_2.csv',header=False,index = False, encoding='utf-8')
